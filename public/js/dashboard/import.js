...
// Snapshot import parsing, validation display, import status formatting, and import RPC submission.
...
function setImportResult(kind, content) {
    ...
    // Prefer text updates; if caller passes a Node/Fragment, append it.
    if (content instanceof Node) {
        importResult.innerHTML = '';
        importResult.appendChild(content);
    } else {
        importResult.textContent = String(content || '');
    }
}
...
function formatImportErrors(errors = []) {
    ...
    if (!Array.isArray(errors) || !errors.length) {
        ...
        p.textContent = 'Unknown validation error.';
        frag.appendChild(p);
        return frag;
    }
    const title = document.createElement('div');
    title.className = 'import-error-title';
    title.textContent = `${errors.length} validation ${errors.length === 1 ? 'error' : 'errors'}`;
    ...
    for (const error of errors.slice(0, 12)) {
        ...
        code.textContent = error.path || 'snapshot';
        li.appendChild(code);
        li.appendChild(document.createTextNode(`: ${error.message || 'invalid'}`));
        ul.appendChild(li);
    }
    frag.appendChild(ul);
    ...
    if (errors.length > 12) {
        ...
        more.textContent = `+${errors.length - 12} more errors`;
...
async function handleImportFile(file) {
    ...
    try {
        ...
        setImportResult('error', `Validation RPC error: ${esc(err.message || String(err))}`);
        return;
    }
...
async function importValidatedSnapshot() {
    ...
    try {
        ...
        setImportResult('error', `Import RPC error: ${esc(err.message || String(err))}`);
        return;
    }
