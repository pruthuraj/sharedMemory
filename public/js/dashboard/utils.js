...
// Small helpers for escaping, colors, node geometry, focus math, status, and counters.
...
function updateZoomUiState(active = false) {
    if (zoomHud) {
        zoomHud.textContent = `${Math.round(scale * 100)}%`;
        zoomHud.classList.toggle('active', active);
    }
    document.body.classList.toggle('low-scale-labels', scale <= LOW_SCALE_LABEL_THRESHOLD);
}
...
function connectedComponentKeys(rootKey) {
    const visited = new Set();
    if (!rootKey || !currentEntries[rootKey]) return visited;
...
function updateStatusCount() {
    ...
    // Keep the status text reserved for connection messages; update counters in the legend only
    updateLegend();
}
